...
const CITIES = ['Beijing', 'Shanghai', 'Xian', 'Chengdu'];
const LANGUAGES = [
...
const SPECIALTIES = ['history', 'food', 'nature', 'photography', 'nightlife', 'art', 'family', 'shopping'];
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'center' }}>
        <FormControl sx={{ minWidth: 160 }}>
          ...
          >
            ...
            {CITIES.map((c) => (
              <MenuItem key={c} value={c}>{t(`cities.${c}`)}</MenuItem>
            ))}
          </Select>
        </FormControl>
        ...
        <FormControl sx={{ minWidth: 160 }}>
          <InputLabel>{t('search.specialty')}</InputLabel>
          <Select
          ...
          >
            ...
            {SPECIALTIES.map((s) => (
              <MenuItem key={s} value={s}>{t(`specialties.${s}`)}</MenuItem>
            ))}
          </Select>
        </FormControl>
        ...
        <Button
          ...
          onClick={onSearch}
          sx={{ height: 56, px: 4, bgcolor: '#1a472a', '&:hover': { bgcolor: '#2d5a3f' } }}
        >
          {t('search.search_btn')}
        </Button>
      </Box>
