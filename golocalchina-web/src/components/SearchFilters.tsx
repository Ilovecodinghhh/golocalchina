    ...
    <Paper elevation={2} sx={{ p: 3, mb: 4, borderRadius: 3 }}>
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'center' }}>
        ...
        <FormControl sx={{ minWidth: 160 }}>
          ...
          <Select value={filters.specialty || ''} label={t('search.specialty')} onChange={(e) => update('specialty', e.target.value)}>
            <MenuItem value="">All Specialties</MenuItem>
            {SPECIALTIES.map((s) => <MenuItem key={s} value={s}>{t(`specialties.${s}`)}</MenuItem>)}
          </Select>
        </FormControl>
