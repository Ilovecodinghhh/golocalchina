...
// High-quality China scenic photos from Unsplash (free to use)
const HERO_IMAGES = [
...
export default function HomePage() {
  ...
  return (
    <Box>
      {/* Hero — full-bleed with rotating scenic backgrounds */}
      <Box sx={{ position: 'relative', height: { xs: '80vh', md: '90vh' }, overflow: 'hidden' }}>
        ...
        <Container maxWidth="md" sx={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'center' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2 }}>
            <img src="/logo.svg" alt="GoLocalChina" style={{ height: 48, filter: 'brightness(10)' }} />
          </Box>
          ...
          <Box>
            <Button
              ...
              sx={{
                bgcolor: '#DC2626', color: 'white', fontWeight: 700, px: 5, py: 1.5,
        ...
        <Box sx={{ position: 'absolute', bottom: 60, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 1 }}>
          {HERO_IMAGES.map((_, idx) => (
            <Box key={idx} onClick={() => setHeroIdx(idx)} sx={{
              ...
              bgcolor: idx === heroIdx ? '#DC2626' : 'rgba(255,255,255,0.5)',
        ...
        <Grid container spacing={3}>
          {Object.entries(CITY_IMAGES).map(([city, data]) => (
            <Grid item xs={6} md={4} key={city}>
              <Paper
                component={Link} to={`/guides?city=${city}`}
                sx={{
                  position: 'relative', height: { xs: 200, md: 260 }, borderRadius: 3, overflow: 'hidden',
                  textDecoration: 'none', display: 'block',
                  '&:hover img': { transform: 'scale(1.1)' },
                  '&:hover .overlay': { background: 'linear-gradient(to top, rgba(220,38,38,0.8) 0%, rgba(0,0,0,0.2) 100%)' },
                }}
              >
                <Box component="img" src={data.img} alt={city} sx={{
      ...
      <Box sx={{ bgcolor: '#FEF2F2', py: 8 }}>
        <Container maxWidth="lg">
          ...
          <Grid container spacing={4}>
            {[
              { icon: <SearchIcon sx={{ fontSize: 40 }} />, num: '01', title: 'Browse & Choose', desc: 'Search guides by city, language, and specialty. Read real reviews. Certified guides have verified licenses.' },
              ...
              <Grid item xs={12} md={4} key={i}>
                <Box sx={{ textAlign: 'center', px: 2 }}>
                  <Typography variant="h1" sx={{ fontWeight: 900, color: '#DC2626', opacity: 0.15, fontSize: 80, lineHeight: 1 }}>
                    {step.num}
                  </Typography>
      ...
      <Box sx={{ bgcolor: '#DC2626', color: 'white', py: 6 }}>
        <Container maxWidth="lg">
          <Grid container spacing={4} justifyContent="center">
            {[
              { num: '6', label: 'Cities' },
              { num: 'Free', label: 'No Platform Fees' },
              { num: '100%', label: 'Direct to Guide' },
              { num: '✓', label: 'License Verified' },
            ].map((stat, i) => (
      ...
      <Box sx={{ py: 8, textAlign: 'center' }}>
        <Container maxWidth="sm">
          ...
          <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
            No sign-up required to browse. Find your guide in minutes.
          </Typography>
          <Button
